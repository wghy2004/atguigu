...
KE.setting = {
	...
	items : [
		...
		'flash', 'media', 'advtable', 'hr','pagebreak', 'emoticons', 'link', 'unlink', '|', 'about'
	],
	colorTable : [
...
KE.util = {
	...
	getData : function(id, wyswygMode) {
		...
		//kingeric修改
	    var con = this.toData(id, g.iframeDoc.body.innerHTML);
		/*过滤word格式*/		
		con = con.replace(/<meta(\n|.)*?>/ig, "");
		con = con.replace(/<style[^>]*>(\n|.)*?<\/style>/ig, "");
		con = con.replace(/<script[^>]*>(\n|.)*?<\/script>/ig, "");
		con = con.replace(/<w:[^>]+>(\n|.)*?<\/w:[^>]+>/ig, "");
		con = con.replace(/<xml>(\n|.)*?<\/xml>/ig, "");
		con = con.replace(/\r\n|\n|\r/ig, "");
		
		con=con.replace(/<div?[^>]*?>pagebreak<\/div\s?>/ig, "<!--pagebreak-->");
	...
	getSrcData : function(id) {
		...
		//kingeric修改
	    var con = g.iframeDoc.body.innerHTML;
		con=con.replace(/<div?[^>]*?>pagebreak<\/div\s?>/ig, "<!--pagebreak-->");
		return con;
	...
	setData : function(id) {
		var g = KE.g[id];
		if (g.srcTextarea){
		   g.srcTextarea.value = this.getData(id);
		}
	},
	focus : function(id) {
		var g = KE.g[id];
	...
	insertHtml : function(id, html) {
		...
		if (KE.browser.IE) {
			...
			if (g.range.item) {
				...
				var elm=g.range.parentElement();
				
				//kingeric修改，使pagebreak嵌套在其他元素中
				if(html.toLowerCase()=='<div class="pagebreak">pagebreak</div>'){
				    if(elm.nodeName=='BODY'){
				         g.range.pasteHTML(html);	
				    }else{
					    var elm2 = null;
				        while(elm.nodeName!="BODY"){ elm2 = elm;elm = elm.parentNode;}
					     elm2.insertAdjacentHTML("afterEnd", html);
				    }
					g.range.collapse(true); 
				}else{
					g.range.pasteHTML(html);
				}
				//g.range.pasteHTML(html);
...
KE.html = function(id, val) {
	if (val === undefined) {
		...
		//alert(KE.util.setFullHtml(id, val);)
		KE.focus(id, 'end');
	}
};
...
KE.plugin['pagebreak'] = {
	click : function(id) {
		KE.util.selection(id);
		KE.util.insertHtml(id, '<div class="pagebreak">pagebreak</div>');
		KE.util.focus(id);
	}
};
...
KE.plugin['source'] = {
	click : function(id) {
		var g = KE.g[id];
		if (!g.wyswygMode) {//设计模式
		    
			//kingeri 修改
			var con = g.newTextarea.value;
			con=con.replace(/<!--pagebreak-->/ig,'<div class="pagebreak">pagebreak</div>');
			KE.util.setFullHtml(id, con);
			...
			//kingeri 修改
			var con = KE.util.getData(id);
			con=con.replace(/<div?[^>]*?>pagebreak<\/div\s?>/ig, "<!--pagebreak-->");
			g.newTextarea.value=con;
